...
def check_site_result(site_no, site_nm):
    ...
    try:
        ...
        for product in hotdl_list:
            ...
            if KEYWORD in name and stock and int(stock) > 0:
                return {"site_nm": site_nm, "name": name, "price": price, "stock": stock}
...
def main():
    ...
    for site in sites:
        ...
        if result:
            print(f"✅ {site_nm} | {result['stock']}개")
            available.append(result)
        else:
            print(f"❌ {site_nm} 품절")
        time.sleep(0.3)

    print(f"\n✅ 전국 재고 체크 완료! 재고 있는 지점: {len(available)}개\n")

    # 결과 한 번에 카카오톡 전송
    if available:
        lines = [f"🟢 요시 팝콘통 재고 있는 지점!\n"]
        for r in available:
            lines.append(f"📍 {r['site_nm']} ({r['stock']}개)")
        lines.append(f"\n💰 가격: {available[0]['price']}원")
